...
import { verificationOutcomeChartColor } from "@/lib/utils/statusChartColors";
import { humanizeEnumValue } from "@/lib/utils/statusStyles";
import type { NamedCount } from "@/types/dashboard";

export function VerificationOutcomeChart({ data }: { data: NamedCount[] }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={chartData} margin={{ top: 4, right: 8, left: -16, bottom: 0 }} barCategoryGap={4}>
        ...
        <Bar dataKey="count" name="Decisions" radius={[4, 4, 0, 0]} maxBarSize={40}>
          {chartData.map((entry) => (
            <Cell key={entry.name} fill={verificationOutcomeChartColor(entry.name)} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
