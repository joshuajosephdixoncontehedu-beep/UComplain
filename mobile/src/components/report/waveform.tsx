...
/** progress (0–1): bars up to that point render solid to reflect real playback position. */
export function Waveform({ progress = 0 }: { progress?: number }) {
  const filledCount = Math.round(progress * BARS.length);
  return (
    <View className="flex-row items-center gap-[3px]">
      {BARS.map((h, i) => (
        <View key={i} className={`w-[3px] rounded-full ${i < filledCount ? 'bg-brand' : 'bg-brand/40'}`} style={{ height: h }} />
      ))}
    </View>
  );
}
